...
%% (Re-pointed 17 Aug 2026 from OAJPE: TPWRS traditional route is US$0 and the
%%  companion paper submits to the same venue. Hard cap 10 pp incl. references
%%  at initial submission -- IEEEtran two-column, the class already in use.)
%%
...
\markboth{Draft to be submitted to the IEEE Transactions on Power Systems}%
...
connection-independent residual, at about 210~VAr per connection (Section~V);
split by the registry's consumer classes, the residential connections---which
hold no power-factor-correction plant---carry a rate inside the bottom-up
estimate. We
...
balanced panel, if anything, understates the national drift~\cite{hume_extended}.
...
swing---and, since a static bias drifts only as the load grows, at $\Delta P\sin\beta$ for any $\beta$: $\Delta P=280$~MW, three times short of the swing. The stronger defence: it is largest in the loaded window while the
...
the six \emph{decoupled} points (the \emph{clean} cohort), and more strictly the
...
points serve about 1.5~million connections---two-thirds of the national fleet the
bottom-up estimate counts---so the 369~MVAr rise normalises to about
...
\begin{figure}[!t]
\centering
\input{../replication/figures/07_class_split.tex}
...
cable---about 190~VAr on Section~IV-C's physics, low-voltage cable being
...
classes carry about half the panel's deepening each despite the six-to-one
...
The measurement gives that correction a concrete, portable form: a net standing shunt term, capacitive by about 210~VAr per connection over 2013--2025 (Section~V-C), alongside whatever power-factor archetype represents the operating draw. It arrives behind existing connections rather than with demand growth, and archetypes calibrated even a decade ago predate it entirely.
...
archived package~\cite{hume_extended}.
...
% entries with bibliographic note fields stripped, to fit the TPWRS 10-page cap.
